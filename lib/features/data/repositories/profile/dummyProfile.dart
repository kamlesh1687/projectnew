...
class DummyProfile extends ProfileRepo {
  ...
  Future<ErrorHandle<UseR>> creatNewUser({String bio, String userName}) {
    return Future.delayed(Duration(seconds: 5)).then((value) {
      ...
      print(randomBool);
      if (randomBool) {
        return ErrorHandle.noError<UseR>(fakeUser);
      } else {
        return ErrorHandle.onError<UseR>('Sorry,your email have some issues');
      }
    });
  }
  ...
  Future<ErrorHandle<UseR>> getData({String uId}) {
    return Future.delayed(Duration(seconds: 5)).then((value) {
      ...
      return ErrorHandle.noError<UseR>(fakeUser);
      //} else {
      //  return ErrorHandle.onError<UseR>('Error');
      //}
    });
  }
